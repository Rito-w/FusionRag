...
class SmartBaseline:
    ...
    def smart_fusion(self, query: Query, bm25_results: List[RetrievalResult], 
        ...
        # 检查查询长度和类型，微调RRF参数
        if len(query_text.split()) > 12:  # 很长查询
            strategy = 'rrf_very_long'
            fusion_results = self._rrf_fusion_with_k(bm25_results, vector_results, k=35)
            
        elif len(query_text.split()) > 8:  # 长查询
            strategy = 'rrf_long_query'
            fusion_results = self._rrf_fusion_with_k(bm25_results, vector_results, k=45)
            
        elif features.query_type.value == 'entity':
            # 实体查询：稍微偏向BM25
            strategy = 'rrf_entity'
            fusion_results = self._rrf_fusion_with_k(bm25_results, vector_results, k=55)
            
        elif features.query_type.value == 'semantic':
            # 语义查询：优化k值
            if len(query_text.split()) > 6:
                strategy = 'rrf_semantic_long'
                fusion_results = self._rrf_fusion_with_k(bm25_results, vector_results, k=50)
            else:
                strategy = 'rrf_semantic_short'
                fusion_results = self._rrf_fusion_with_k(bm25_results, vector_results, k=65)
            
        elif features.query_type.value == 'keyword':
            # 关键词查询：稍微偏向向量
            strategy = 'rrf_keyword'
            fusion_results = self._rrf_fusion_with_k(bm25_results, vector_results, k=75)
    ...
    def _rrf_fusion_with_k(self, bm25_results: List[RetrievalResult], 
