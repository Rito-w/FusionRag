...
在信息检索中，多检索器融合已成为提高检索性能的重要技术。传统观点认为，更复杂的融合策略能带来更好的结果，促使研究人员开发日益复杂的方法。然而，我们的研究通过提出"Less is More"（少即是多）视角来挑战这一假设。

实验结果揭示了一个重要发现：**在本研究的特定数据集和实验条件下，简单的线性融合方法可以达到或超越复杂方法的性能**。具体而言，简单线性融合在FIQA、Quora和SciDocs数据集上分别比标准RRF方法提升8.2%、7.2%和10.9%的MRR性能，在SciFact数据集上也有2.2%的提升。重要的是，消融实验表明移除复杂组件（如查询分析器）可以在某些情况下提升性能。此外，简单方法在计算效率方面具有显著优势，推理速度明显快于复杂方法。

这些发现为"简单方法在特定场景下可能更有效"的假设提供了实证支持，挑战了"复杂性等于更好"的普遍假设，并为实际系统设计提供了重要的指导原则。
...
本研究源于我们在构建生产级多检索器融合系统时的实际经验。受近年来学术界提出的众多复杂融合策略启发，我们对最新论文进行了系统分析，发现了几个主要创新方向：
...
然而，在6个BEIR数据集[11]上的系统性实验评估中，我们得到了一个重要的发现：**在本研究的实验条件下，简单的线性融合方法在多数数据集上表现优于我们设计的复杂自适应系统**。
...
这些发现在特定实验条件下对我们的初始设计理念提出了挑战，更重要的是对信息检索领域"复杂即更好"的普遍假设提出了质疑，为"Less is More"（少即是多）的设计理念提供了实证支持。
...
近年来，生成式信息检索（Generative Retrieval, GR）作为信息检索领域的一个重要发展方向受到广泛关注，它将传统的"检索-排序"范式转变为"生成-检索"范式。最新的综述研究[31,35]系统梳理了RAG技术的发展脉络，特别关注了生成式检索在RAG系统中的应用。
...
我们使用标准的文本预处理流程，包括分词、小写化、停用词过滤和词干提取。

#### 3.2.2 稠密检索器 (E5-large-v2)

我们选择E5-large-v2[10]作为稠密检索器，这是一个基于Transformer架构的预训练文本嵌入模型，在多个文本相似度和检索任务上都取得了优异的性能。该模型使用1024维嵌入，最大序列长度为512个token。
...
表2的结果颠覆了我们的预期。在6个数据集中，简单的线性融合策略在5个数据集上优于RRF这一中等复杂度方法（FIQA提升8.2%、Quora提升7.2%、SciDocs提升10.9%、SciFact提升2.2%、ArguAna提升0.7%），在1个数据集上与RRF表现相当（NFCorpus）。
...
2. **简单方法的显著优势**: 简单线性融合方法在5个数据集上显著优于RRF这一中等复杂度方法，分别在FIQA、Quora、SciDocs、SciFact和ArguAna数据集上取得了8.2%、7.2%、10.9%、2.2%和0.7%的性能提升。
