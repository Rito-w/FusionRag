...
实验结果揭示了一个有趣的发现：**简单的线性融合方法在特定数据集上可以达到或超越复杂方法的性能**。具体而言，简单线性融合在FIQA、Quora和SciDocs数据集上分别比标准RRF方法提升8.2%、7.2%和10.9%的MRR性能，在SciFact数据集上也有2.2%的提升。重要的是，消融实验表明在某些情况下，移除复杂组件（如查询分析器）可以提升性能，这在SciDocs数据集上表现尤为明显。此外，简单方法在计算效率方面显著优于复杂方法，平均推理时间仅为1.2-1.3ms/查询，而复杂方法需要100ms以上。
...
1. **系统性验证**：首次系统性验证8种融合策略在6个BEIR数据集上的表现，提供了全面的性能分析
...
从表1可以看出，在大多数数据集上，线性融合方法与RRF表现相近或更好。特别是在FIQA、Quora和SciDocs数据集上，简单线性方法明显优于RRF，提升幅度分别达到8.2%、7.2%和10.9%。这初步表明简单的线性融合方法在多数情况下可以超越更复杂的RRF方法。
...
**表3: 消融实验结果（MRR ± 标准差）**
| 数据集  | RRF基线     | 无查询分析  | 无自适应路由 | 静态权重    | 最佳配置   |
|---------|-------------|-------------|--------------|-------------|------------|
| Quora   | 0.669±0.018 | 0.671±0.019 | 0.669±0.018  | 0.663±0.014 | 无查询分析 |
| SciDocs | 0.294±0.013 | 0.326±0.016 | 0.310±0.014  | 0.290±0.010 | 无查询分析 |
| FIQA    | 0.317±0.012 | 0.324±0.015 | 0.320±0.013  | 0.316±0.009 | 无查询分析 |



消融实验的结果提供了有价值的洞察：在SciDocs数据集上，移除查询分析器并使用优化权重后，性能从RRF基线的0.294±0.013提升到0.326±0.016，提升幅度达到10.9%。在Quora数据集上，移除查询分析器后性能也有轻微提升，从0.669±0.018提升到0.671±0.019。这表明在某些情况下，简化系统架构并优化基础参数可能比增加复杂组件更有效。在FIQA数据集上，简化的效果虽然相对较小，但仍有正向趋势，说明复杂组件的影响具有数据集特异性。
...
1. **简单方法的竞争力**: 在6个数据集中，简单线性融合方法在4个数据集上表现优于RRF方法，其中在FIQA、Quora和SciDocs数据集上取得了显著的性能提升。

2. **复杂组件的有限贡献**: 消融实验表明，在某些情况下（如SciDocs数据集），移除复杂组件（如查询分析器）可以显著提升性能。
